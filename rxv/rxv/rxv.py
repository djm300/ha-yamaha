...
ZoneBVolumeLevel = '<Volume><Zone_B><Lvl>{value}</Lvl></Zone_B></Volume>'
VolumeLevelValue = '<Val>{val}</Val><Exp>{exp}</Exp><Unit>{unit}</Unit>'
VolumeMute = '<Volume><Mute>{state}</Mute></Volume>'
ZoneBVolumeMute = '<Volume><Zone_B><Mute>{state}</Mute></Zone_B></Volume>'
SpeakerControl = '<Speaker_Preout><Speaker_AB><{speaker}>{state}</{speaker}></Speaker_AB></Speaker_Preout>'
...
class RXV(object):
    ...
    def __init__(self, ctrl_url, model_name="Unknown",
        ...
        # This is override-specific instrumentation: capture the constructor
        # inputs once so zone/controller mismatches can be traced from startup.
        logger.debug(
            "Initializing RXV: ctrl_url=%s unit_desc_url=%s model_name=%s friendly_name=%s serial_number=%s zone=%s timeout=%s",
            self.ctrl_url, self.unit_desc_url, self.model_name,
            self.friendly_name, self.serial_number, self._zone,
            self.http_timeout
        )
    ...
    def _discover_features(self):
        ...
        try:
            logger.debug("Fetching Yamaha descriptor XML from %s", self.unit_desc_url)
            ...
            logger.debug(
                "Loaded descriptor XML for zone=%s from %s (%d bytes)",
                self._zone, self.unit_desc_url, len(desc_xml)
            )
    ...
    def _main_zone_request(self, command, request_text):
        payload = Zone.format(request_text=request_text, zone="Main_Zone")
        request_text = YamahaCommand.format(command=command, payload=payload)
        try:
            res = self._session.post(
                self.ctrl_url,
    ...
    def _speaker_tag(self):
        return "Speaker_B" if self.zone == "Zone_B" else "Speaker_A"

    @property
    def basic_status(self):
        if self.zone == "Zone_B":
            response = self._main_zone_request('GET', BasicStatusGet)
            ...
            mute = response.find("Main_Zone/Basic_Status/Volume/Zone_B/Mute").text
            volume = response.find("Main_Zone/Basic_Status/Volume/Zone_B/Lvl/Val").text
        else:
    ...
    def on(self):
        ...
        response = self._main_zone_request('GET', request_text) \
            if self.zone == "Zone_B" else self._request('GET', request_text)
        power = response.find("Main_Zone/Power_Control/Power" if self.zone == "Zone_B"
                              else "%s/Power_Control/Power" % self._zone).text
    ...
    def on(self, state):
        ...
        response = self._main_zone_request('PUT', request_text) \
            if self.zone == "Zone_B" else self._request('PUT', request_text)
        return response
    ...
    def enabled(self):
        response = self._main_zone_request('GET', BasicStatusGet)
        speaker = response.find(
            "Main_Zone/Basic_Status/Speaker_Preout/Speaker_AB/%s" %
            self._speaker_tag()
        )
        return speaker is not None and speaker.text == "On"

    @enabled.setter
    def enabled(self, state):
        assert state in [True, False]
        new_state = "On" if state else "Off"
        request_text = SpeakerControl.format(
            speaker=self._speaker_tag(),
            state=new_state,
        )
        self._main_zone_request('PUT', request_text)
    ...
    def inputs(self):
        if not self._inputs_cache:
            ...
            if self.zone == "Zone_B":
                res = self._main_zone_request('GET', request_text)
            else:
    ...
    def zone(self, zone_name):
        ...
        logger.debug("Switching RXV zone from %s to %s", self._zone, zone_name)
        self._zone = zone_name

    def zones(self):
        if self._zones_cache is None:
            # Zone enumeration is the critical path for this override, so log
            # how the descriptor XML is normalized into Yamaha zone names.
            logger.debug("Building Yamaha zones cache from descriptor for %s", self.ctrl_url)
            zones = []
            for menu in self._desc_xml.findall('.//*[@Func="Subunit"]'):
                zone = menu.get("YNC_Tag")
                if menu.get("Title_1") == "Zone B":
                    zone = "Zone_B"
                if zone not in zones:
                    logger.debug(
                        "Discovered Yamaha zone candidate: ync_tag=%s title=%s normalized_zone=%s",
                        menu.get("YNC_Tag"), menu.get("Title_1"), zone
                    )
                    zones.append(zone)
            self._zones_cache = zones
            logger.debug("Yamaha zones cache populated: %s", self._zones_cache)
        else:
            logger.debug("Using cached Yamaha zones for %s: %s", self.ctrl_url, self._zones_cache)
        return self._zones_cache

    def zone_controllers(self):
        """Return separate RXV controller for each available zone."""
        # Log each cloned controller because Home Assistant creates one entity
        # per returned zone controller, and that is where zone-loading diverges.
        logger.debug("Creating zone controllers for %s", self.ctrl_url)
        controllers = []
        for zone in self.zones():
            zone_ctrl = copy.copy(self)
            zone_ctrl.zone = zone
            logger.debug(
                "Created zone controller: ctrl_url=%s zone=%s serial=%s object_id=%s",
                zone_ctrl.ctrl_url, zone_ctrl.zone, zone_ctrl.serial_number,
                hex(id(zone_ctrl))
            )
            controllers.append(zone_ctrl)
        logger.debug("Returning %d zone controllers for %s", len(controllers), self.ctrl_url)
        return controllers
    ...
    def volume(self):
        if self.zone == "Zone_B":
            request_text = ZoneBVolumeLevel.format(value=GetParam)
            response = self._main_zone_request('GET', request_text)
            vol = response.find('Main_Zone/Volume/Zone_B/Lvl/Val').text
        else:
    ...
    def volume(self, value):
        ...
        if self.zone == "Zone_B":
            request_text = ZoneBVolumeLevel.format(value=volume_val)
            self._main_zone_request('PUT', request_text)
            return
    ...
    def mute(self):
        if self.zone == "Zone_B":
            request_text = ZoneBVolumeMute.format(state=GetParam)
            response = self._main_zone_request('GET', request_text)
            mute = response.find('Main_Zone/Volume/Zone_B/Mute').text
        else:
    ...
    def mute(self, state):
        ...
        new_state = "On" if state else "Off"
        if self.zone == "Zone_B":
            request_text = ZoneBVolumeMute.format(state=new_state)
            response = self._main_zone_request('PUT', request_text)
        else:
