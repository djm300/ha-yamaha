...
On some Yamaha models, additional listening zones are exposed as speaker-switched
outputs (for example ``Zone_B``). In that case, receiver power remains ``rx.on``,
while per-zone enable state is available through ``enabled``::

  >>> zones = rx.zone_controllers()
  >>> zone_b = [z for z in zones if z.zone == "Zone_B"][0]
  >>> rx.on = True
  >>> zone_b.enabled = True
  >>> zone_b.volume = -60.0
  >>> zone_b.enabled = False
